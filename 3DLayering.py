...
def Plot_Binary(sim, t, is3d=False):    
    ...
    sp_alpha =  0.1 if t < 600 else max(0.0, 0.1-0.01*(t-600))
    ...
    ax.text(7, 0, 1, r"$\mathit{i = 20\degree}$", alpha=text_alpha)
    ax.text(1, 0, 7, r"$\mathit{i = 70\degree}$", alpha=text_alpha)
...
def anim(t):
    ...
    if t > 400:
        ...
        inc_alpha = 1.0 if t < 580 else max(0.0, 1.0-0.1*(t-580))
        ax.text(0, 9, 9, r"$\mathit{{i = {:.0f}\degree}}$".format(min(i_t, 180)), fontsize=16, alpha=inc_alpha)
        for i in range(18):
            ...
            # Scene 4: Project sphere onto 2D surface
            else:
...
time_s1 = Interpolation(0,300,500)
time_s2 = Interpolation(300,390,240)
time_s3 = Interpolation(400,580,240)
time_s4 = Interpolation(600,840,240)

# Plots for Scenes 3 and 4
...
times = np.concatenate( (time_s1,time_s2,time_s3, time_s4) )
